...
public class Main {
    ...
    private static void moveKnights(int kidx, int dir) {
    	...
    	if (canGo) {
    		...
    		for (int i = 0; i < knights.size(); i++) tmp.add(knights.get(i));
    		for (int i = 0; i < moved.size(); i++) tmp.remove(knights.get(moved.get(i)[0]));
    		for (int i = 0; i < tmp.size(); i++) {
    			Knight now = tmp.get(i);
    			
    			for (int i2 = 0; i2 < now.height; i2++) {
    				for (int j2 = 0; j2 < now.width; j2++) {
    					kmap[now.r+i2][now.c+j2] = i+1;
    				}
    			}
    		}
